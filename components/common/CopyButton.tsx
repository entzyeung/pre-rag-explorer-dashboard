    ...
    >
      {copied ? (
        <>
          ...
          {!iconOnly && <span className="text-[10px] font-bold uppercase tracking-tight">Copied</span>}
        </>
      ) : (
        <>
          <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {!iconOnly && label && <span className="text-[10px] font-bold uppercase tracking-tight">{label}</span>}
        </>
      )}
    </button>
  );
